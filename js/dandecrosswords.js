(function ($, Drupal) {
  ...
  Drupal.behaviors.crossword = {
    attach: function (context, settings) {
      ...
      if( $('#crossword', context).length > 0 ){

        var fileName = settings.fileName.url;
        ...
        // //get the event listener and upload function on the file input
        ...
        //         state = {
        //         orientationAcross: true,
        //         activeSquare: { row: null, column: null },
        //         activeClue: null,
        //         crossword: null,
        //         answers: null
        //       }
        //         state.crossword = createCrosswordFromText(reader.result);
        //         $('#content').html("");
        ...
        //then call the createMarkup function
        function loadPuzzle( fileName ){
          if( fileName.substr(fileName.length - 4) === '.txt'){
          ...
          } else if(fileName.substr(fileName.length - 4) === '.puz'){
            ...
            xhr.onload = function(e) {
              ...
              var crossword = new Crossword(makeTextFromPuz(new Uint8Array(this.response)));
              state = new Game(crossword);
              createMarkup(state.crossword);
            };
            xhr.send();
